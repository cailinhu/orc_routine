package com.clh.capture;

import com.clh.bean.OrcResult;
import com.clh.bean.WordsResult;
import com.clh.util.BaiduOcrUtil;
 ...
 *
 */
public class CaptureTool extends Frame implements MouseListener, MouseMotionListener, KeyListener {
	...
	public  int changeFirstPointX, changeFirstPointY, changeWidth, changeHeight;
	...
	public CaptureTool(JFrame frame, JTextArea textArea) {
		// 取得屏幕大小
		try {
			//目的是:让主窗口先隐藏
			Thread.sleep(500);
		...
		copyItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ImageIO.write(sbi, "jpeg", os);
					OrcResult orcResult = BaiduOcrUtil.orcResult(os);
					java.util.List<WordsResult> words_result = orcResult.getWords_result();
					for (WordsResult wordsResult : words_result) {
						builder.append(wordsResult.getWords() + "\n");
					}
...
//				try {
//					ImageIO.write(sbi, "jpeg", new File("screenshot.jpeg"));
...
//	}
	...
	private class MyTakePicture implements ActionListener {
		// 保存图片
		public void actionPerformed(ActionEvent e) {
			FileDialog fileDialog = new FileDialog(CaptureTool.this, "保存截图", FileDialog.SAVE);
			...
			if (dir != null && !"".equals(dir) && (fileName == null || "".equals(fileName))) {
				JOptionPane.showMessageDialog(CaptureTool.this, "Error file!", "Error",
			...
			if (dirFile.exists()) {
				if (!dirFile.isDirectory()) {
					JOptionPane.showMessageDialog(CaptureTool.this, "\"" + dir + "\" is not a directory!",
							"Error", JOptionPane.ERROR_MESSAGE);
				}
			} else {
				int ret = JOptionPane.showConfirmDialog(CaptureTool.this, "\"" + dir
